...
namespace Zadaca1.Algoritmi
{
    public class Pretrage
    {
        ...
        public static int interpolacijskaPretraga(IList<string> niz, string element)
        {
            int min = 0, max = niz.Count() - 1;
            ...
            while (min <= max && element >= niz[min] && element <= niz[max])
            {
                ...
                int pos = (min + ((max - min) / (niz[max] - niz[min]) * (element - niz[min]));
